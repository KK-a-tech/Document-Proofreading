...
async function setting_service(text) {
  const url =
    'https://jlp.yahooapis.jp/KouseiService/V2/kousei?appid=' +
...
function make_view(text, obj) {
  [...obj.result.suggestions].reverse().forEach((s) => {
    ...
    text =
      `${pre}<span title="${note}"><del>${s.word}</del><ins>${sug}</ins>` +
...
function change_suggestion_style(e) {
  e.querySelector('del').style.color = e.querySelector('input').checked
    ? 'gray'
    : 'black';
  e.querySelector('ins').style.color = e.querySelector('input').checked
    ? 'black'
    : 'gray';
  convert_view_to_text();
}
